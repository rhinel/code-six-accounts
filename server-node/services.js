...
module.exports = {
  ...
  login: (req, res, callback) => {
    ...
    if (!req.body.name) {
      ...
      // 生成新token
      const _tokenTime = new Date().getTime()
      const _token = md5(req.ip + req.body.name + req.body.pwd + _tokenTime)
      let _userId
      // db类操作，异步同步化
      ...
      // 2数据库查询用户名和密码校验
        .then(() => new Promise((resolved, rejectd) => {
          db.pool
      ...
      // 3查出已有的登陆态，更新状态
        .then((dbData) => db.redisGetKeys(`${dbData[0].userId}$*`)
          // 旧token失效？
          .then((reKeysData) => {
          ...
          .then(() => db.redisSet(`${_userId.toString()}$${_token}`, _userId.toString(), 1800)))
      // 4返回token
        .then(() => Promise.reject({
          type: true,
          data: _token,
