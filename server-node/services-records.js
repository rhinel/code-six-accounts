...
const fixNum = (n) => Math.round(n * 100) / 100
      ...
      db.pool
        .getConnection((err, connection) => {
          ...
          connection.beginTransaction(async (err) => {
            ...
            // 1查询类目
            const check = await (() => new Promise((resolved, rejectd) => {
            ...
            // 1更新
            const checkResults = await (() => new Promise((resolved, rejectd) => {
            ...
            if (check[0].calc == 1) {
              // 2查询日期
              const checkDate = await (() => new Promise((resolved, rejectd) => {
              ...
              // 2更新日期
              const checkDateCalc = await (() => new Promise((resolved, rejectd) => {
              ...
              // 2更新统计
              const checkUser = await (() => new Promise((resolved, rejectd) => {
            ...
            // 3插入数据
            const results = await (() => new Promise((resolved, rejectd) => {
      ...
      db.pool
        .getConnection((err, connection) => {
          ...
          connection.beginTransaction(async (err) => {
            ...
            // -------------------------------------------------------------------------------------------------------------------
            // 1查询自身原有记录
            const checkRecord = await (() => new Promise((resolved, rejectd) => {
              connection.query('SELECT * FROM `accounts_records` WHERE `recordId` = ? AND `status` = 1 AND `userId` = ?', [
                req.body.recordId,
                req.userId,
              ], (err, results, fields) => {
            ...
            // -------------------------------------------------------------------------------------------------------------------
            // 2查询原有记录类目
            const checkType = await (() => new Promise((resolved, rejectd) => {
              connection.query('SELECT * FROM `accounts_types` WHERE `typeId` = ? AND `status` = 1 AND `userId` = ?', [
            ...
            if (checkType[0].calc == 1) {
              ...
              // 2判断
              const ids = 'UPDATE `accounts_dates` SET `increased` = ?, `reduce` = ? WHERE `date` = ? AND `status` = 1 AND `userId` = ?'
              const idd = [
            ...
            if (checkRecord[0].typeId == req.body.typeId) {
              ...
              updateType = await (() => new Promise((resolved, rejectd) => {
                connection.query('SELECT * FROM `accounts_types` WHERE `typeId` = ? AND `status` = 1 AND `userId` = ?', [
                  req.body.typeId,
                  req.userId,
                ], (err, results, fields) => {
            ...
            // 3更新update类目
            const updateResults = await (() => new Promise((resolved, rejectd) => {
              connection.query('UPDATE `accounts_types` SET `increased` = ?, `reduce` = ? WHERE `typeId` = ? AND `status` = 1 AND `userId` = ?', [
            ...
            if (updateType[0].calc == 1) {
              // 3查询日期
              const updateDate = await (() => new Promise((resolved, rejectd) => {
                connection.query('SELECT * FROM `accounts_dates` WHERE `date` = ? AND `status` = 1 AND `userId` = ?', [
                  req.body.date,
                  req.userId,
                ], (err, results, fields) => {
              ...
              // 3更新日期
              const updateDateCalc = await (() => new Promise((resolved, rejectd) => {
                connection.query(ids, idd, (err, results, fields) => {
                  if (err) {
                    connection.release()
                    return connection.rollback(rejectw({
              ...
              // 3更新统计
              const updateUser = await (() => new Promise((resolved, rejectd) => {
                connection.query('UPDATE `users` SET `count` = `count` + ? WHERE `status` = 1 AND `userId` = ?', [
                  fixNum(Number(req.body.increased) - Number(req.body.reduce)),
                  req.userId,
                ], (err, results, fields) => {
